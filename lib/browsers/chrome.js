// h/t https://github.com/prerender/prerender/pull/779/commits/d49ceac453fc490415ed59967943bb50c8537129
...
chrome.spawn = function (options) {
	return new Promise((resolve, reject) => {
		...
		this.chromeChild = spawn(location, this.options.chromeFlags || [
			...
			'--hide-scrollbars',
		// following line commented out as per:
		// https://github.com/prerender/prerender/issues/755
	...
	return new Promise((resolve, reject) => {
		...
		let connect = () => {
			CDP.Version({ port: this.options.browserDebuggingPort }).then((info) => {
				...
				// this.webSocketDebuggerURL = info.webSocketDebuggerUrl || 'ws://127.0.0.1:' + this.options.browserDebuggingPort + '/devtools/browser';
		...
		const connectToBrowser = async (target, port) => {
			...
			for(;;) {
				try {
					...
					util.error(`[chrome] Cannot connect to browser port=${port} remainingRetries=${remainingRetries}`, err);
