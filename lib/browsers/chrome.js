...
chrome.spawn = function (options) {
	return new Promise((resolve, reject) => {
		...
		if (!fs.existsSync(location)) {
			util.log('[chrome] unable to find Chrome install. Please specify with chromeLocation');
			return reject();
		}
	...
	return new Promise((resolve, reject) => {
		...
		let connect = () => {
			CDP.Version({ port: this.options.browserDebuggingPort }).then((info) => {
				...
				util.log('[chrome] retrying connection to Chrome...');
		...
		const connectToBrowser = async (target, port) => {
			...
			for(;;) {
				try {
					...
					util.log(`[chrome] Cannot connect to browser port=${port} remainingRetries=${remainingRetries}`, err);
...
chrome.setUpEvents = async function (tab) {
	...
	Security.certificateError(({ eventId }) => {
		Security.handleCertificateError({
			...
			util.log('[chrome] error handling certificate error:', err);
		});
	});
		...
		if (params.redirectResponse) {
			...
			if (tab.prerender.initialRequestId === params.requestId && !tab.prerender.followRedirects && !this.options.followRedirects) {
				...
				Page.stopLoading().catch((err) => {
					util.log(`[chrome] unable to stop loading page (redirect), url=${params.request.url}`, err);
				});
			}
		}
	});
			...
			.then(() => {
				...
				var checkIfDone = () => {
					...
					this.checkIfPageIsDoneLoading(tab).then((doneLoading) => {
						...
						finished = true;
						util.log('[chrome] Chrome connection closed during request');
				...
				setTimeout(() => {
					if (!finished) {
						finished = true;
						util.log('[chrome] page timed out', tab.prerender.url);
				...
				Page.navigate({
					...
					if (tab.prerender.navigateError && tab.prerender.navigateError !== 'net::ERR_ABORTED') {
						util.log(`[chrome] Navigation error: ${tab.prerender.navigateError}, url=${tab.prerender.url}`);
						Page.stopLoading().catch((err) => {
							util.log(`[chrome] unable to stop loading page, url=${tab.prerender.url}`, err);
						});
					}
					...
					util.log('[chrome] invalid URL sent to Chrome:', tab.prerender.url);
					tab.prerender.statusCode = 504;
					finished = true;
					reject();
				});
			}).catch((err) => {
				util.log('[chrome] unable to load URL', err);
				tab.prerender.statusCode = 504;
...
chrome.checkIfPageIsDoneLoading = function (tab) {
	return new Promise((resolve, reject) => {
		...
		tab.Runtime.evaluate({
			...
			util.log('[chrome] unable to evaluate javascript on the page');
			tab.prerender.statusCode = 504;
		...
		tab.Runtime.evaluate({
			...
			setTimeout(() => {
				...
				tab.Runtime.evaluate({
					...
					util.log('[chrome] unable to evaluate javascript on the page', err);
					tab.prerender.statusCode = 504;
					tab.prerender.errors.push(UnableToEvaluateJavascript);
					reject();
				});
...
chrome.parseHtmlFromPage = function (tab) {
	return new Promise((resolve, reject) => {
		...
		var parseTimeout = setTimeout(() => {
			util.log('[chrome] parse html timed out', tab.prerender.url);
			tab.prerender.statusCode = 504;
		...
		tab.Runtime.evaluate({
			...
			util.log('[chrome] unable to parse HTML', err);
			tab.prerender.statusCode = 504;
...
chrome.captureScreenshot = function (tab, format, fullpage) {
	return new Promise((resolve, reject) => {
		...
		var parseTimeout = setTimeout(() => {
			util.log('[chrome] capture screenshot timed out for', tab.prerender.url);
			tab.prerender.statusCode = 504;
		...
		tab.Page.getLayoutMetrics().then((viewports) => {
			...
			tab.Page.captureScreenshot({
				...
				util.log('[chrome] unable to capture screenshot:', err);
				tab.prerender.statusCode = 504;
...
chrome.printToPDF = function (tab, options) {
	return new Promise((resolve, reject) => {
		...
		var parseTimeout = setTimeout(() => {
			util.log('[chrome] print pdf timed out for', tab.prerender.url);
			tab.prerender.statusCode = 504;
		...
		tab.Page.printToPDF(options).then((response) => {
			...
			util.log('[chrome] unable to capture pdf:', err);
			tab.prerender.statusCode = 504;
