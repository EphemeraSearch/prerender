// h/t PythonicCafe/turicas: https://github.com/prerender/prerender/pull/779/commits/d49ceac453fc490415ed59967943bb50c8537129
...
chrome.spawn = function (options) {
	return new Promise((resolve, reject) => {
		...
		if (!fs.existsSync(location)) {
			util.error('[chrome] unable to find Chrome install. Please specify with chromeLocation');
			return reject();
		}
		...
		this.chromeChild = spawn(location, this.options.chromeFlags || [
			...
			'--no-sandbox',
			'--disable-gpu',
			'--remote-debugging-host=' + this.options.browserDebuggingHost,
...
chrome.onClose = function (callback) {
	util.log('[chrome] Starting child browser shutdown pipeline');
...
chrome.connect = function () {
	return new Promise((resolve, reject) => {
		...
		let timeout = setTimeout(() => {
			if (!connected) {
				util.error('[chrome] not connected');
				reject();
			}
		}, 20 * 1000);

		let connect = () => {
			util.log('[chrome] connecting to Chrome');
			CDP.Version({ host: this.options.browserDebuggingHost, port: this.options.browserDebuggingPort }).then((info) => {
				util.log('[chrome] connected to Chrome', info['User-Agent'], info);
				...
				util.log(`[chrome] Got webSocketDebuggerURL: ${this.webSocketDebuggerURL}`);
				...
				util.error('[chrome] retrying connection to Chrome...', err);
	...
	return new Promise((resolve, reject) => {
		...
		const connectToBrowser = async (host, port, target) => {
			...
			for(;;) {
				try {
					...
					util.error(`[chrome] Cannot connect to browser host=${host}, port=${port}, target=${target}, remainingRetries=${remainingRetries}`, err);
		...
		connectToBrowser(this.options.browserDebuggingHost, this.options.browserDebuggingPort, this.webSocketDebuggerURL)
			.then((chromeBrowser) => {
				...
				util.log('[chrome.js] Error on connectToBrowser (openTab) pipeline', err);
				reject(err);
			});
	});
};
...
chrome.setUpEvents = async function (tab) {
	...
	const userAgent = (
		...
		`${this.originalUserAgent} Prerender (+https://github.com/EphemeraSearch/prerender)`
	);
	...
	Security.certificateError(({ eventId }) => {
		Security.handleCertificateError({
			...
			util.error('[chrome] error handling certificate error:', err);
		});
	});
	...
	Network.requestWillBeSent((params) => {
		...
		if (tab.prerender.logRequests || this.options.logRequests) util.log('[chrome] +', tab.prerender.numRequestsInFlight, params.request.url);

		if (!tab.prerender.initialRequestId) {
			util.log(`[chrome] Initial request to ${params.request.url}`);
		...
		if (params.redirectResponse) {
			...
			if (tab.prerender.initialRequestId === params.requestId && !tab.prerender.followRedirects && !this.options.followRedirects) {
				util.log(`[chrome] Initial request redirected from ${params.request.url} with status code ${params.redirectResponse.status}`);
				...
				Page.stopLoading().catch((err) => {
					util.error(`[chrome] unable to stop loading page (redirect), url=${params.request.url}`, err);
				});
			}
		}
	});
	...
	Network.responseReceived((params) => {
		...
		if (params.requestId == tab.prerender.initialRequestId && !tab.prerender.receivedRedirect) {
			util.log(`[chrome] Initial response from ${params.response.url} with status code ${params.response.status}`);
		...
		if (params.type === "EventSource") {
			tab.prerender.numRequestsInFlight--;
			tab.prerender.lastRequestReceivedAt = new Date().getTime();
			if (tab.prerender.logRequests || this.options.logRequests) util.log('[chrome] -', tab.prerender.numRequestsInFlight, tab.prerender.requests[params.requestId]);
	...
	Network.loadingFinished(({ requestId, timestamp, encodedDataLength }) => {
		...
		if (request) {
			if (tab.prerender.initialRequestId === requestId) {
				util.log(`[chrome] Initial request finished ${request}`);
			}

			tab.prerender.numRequestsInFlight--;
			tab.prerender.lastRequestReceivedAt = new Date().getTime();

			if (tab.prerender.logRequests || this.options.logRequests) util.log('[chrome] -', tab.prerender.numRequestsInFlight, tab.prerender.requests[requestId]);
	...
	Console.messageAdded((params) => {
		...
		if (tab.prerender.logRequests || this.options.logRequests) {
			...
			if (message.level === 'error') {
				util.error('[chrome] level:', message.level, 'text:', message.text, 'url:', message.url, 'line:', message.line);
			} else {
				util.log('[chrome] level:', message.level, 'text:', message.text, 'url:', message.url, 'line:', message.line);
			}
		}
	});
	...
	Log.entryAdded((params) => {
		...
		if (tab.prerender.logRequests || this.options.logRequests) util.log(`[chrome] ${params.entry}`);
	});
			...
			.then(() => {
				...
				var checkIfDone = () => {
					...
					this.checkIfPageIsDoneLoading(tab).then((doneLoading) => {
						...
						finished = true;
						util.error('[chrome] Chrome connection closed during request');
				...
				setTimeout(() => {
					if (!finished) {
						finished = true;
						util.error('[chrome] page timed out', tab.prerender.url);
				...
				Page.navigate({
					...
					if (tab.prerender.navigateError && tab.prerender.navigateError !== 'net::ERR_ABORTED') {
						util.error(`[chrome] Navigation error: ${tab.prerender.navigateError}, url=${tab.prerender.url}`);
						Page.stopLoading().catch((err) => {
							util.error(`[chrome] unable to stop loading page, url=${tab.prerender.url}`, err);
						});
					}
					...
					util.error('[chrome] invalid URL sent to Chrome:', tab.prerender.url);
					tab.prerender.statusCode = 504;
					finished = true;
					reject();
				});
			}).catch((err) => {
				util.error('[chrome] unable to load URL', err);
				tab.prerender.statusCode = 504;
...
chrome.checkIfPageIsDoneLoading = function (tab) {
	return new Promise((resolve, reject) => {
		...
		tab.Runtime.evaluate({
			...
			util.error('[chrome] unable to evaluate javascript on the page');
			tab.prerender.statusCode = 504;
		...
		tab.Runtime.evaluate({
			...
			setTimeout(() => {
				...
				tab.Runtime.evaluate({
					...
					util.error('[chrome] unable to evaluate javascript on the page', err);
					tab.prerender.statusCode = 504;
					tab.prerender.errors.push(UnableToEvaluateJavascript);
					reject();
				});
...
chrome.parseHtmlFromPage = function (tab) {
	return new Promise((resolve, reject) => {
		...
		var parseTimeout = setTimeout(() => {
			util.error('[chrome] parse html timed out', tab.prerender.url);
			tab.prerender.statusCode = 504;
		...
		tab.Runtime.evaluate({
			...
			util.error('[chrome] unable to parse HTML', err);
			tab.prerender.statusCode = 504;
...
chrome.captureScreenshot = function (tab, format, fullpage) {
	return new Promise((resolve, reject) => {
		...
		var parseTimeout = setTimeout(() => {
			util.error('[chrome] capture screenshot timed out for', tab.prerender.url);
			tab.prerender.statusCode = 504;
		...
		tab.Page.getLayoutMetrics().then((viewports) => {
			...
			tab.Page.captureScreenshot({
				...
				util.error('[chrome] unable to capture screenshot:', err);
				tab.prerender.statusCode = 504;
...
chrome.printToPDF = function (tab, options) {
	return new Promise((resolve, reject) => {
		...
		var parseTimeout = setTimeout(() => {
			util.error('[chrome] print pdf timed out for', tab.prerender.url);
			tab.prerender.statusCode = 504;
		...
		tab.Page.printToPDF(options).then((response) => {
			...
			util.error('[chrome] unable to capture pdf:', err);
			tab.prerender.statusCode = 504;
