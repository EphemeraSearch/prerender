...
if (!LOG_LEVELS.includes(LOG_LEVEL)) {
	...
	util.log(`[server.js] Log level: ${LOG_LEVEL}`);
};
...
server.start = function() {
	util.log('[server.js] Starting Prerender');
	...
	this.startPrerender().then(() => {
		...
		process.on('SIGINT', () => {
			this.killBrowser();
			setTimeout(() => {
				util.log('[server.js] Stopping Prerender');
				process.exit();
			}, 500);
		});
...
server.startPrerender = function() {
	return new Promise((resolve, reject) => {
		this.spawnBrowser().then(() => {
			...
			util.log(`[server.js] Started ${this.browser.name}: ${this.browser.version}`);
			return this.firePluginEvent('connectedToBrowser');
		}).then(() => resolve())
		.catch((err) => {
			util.error(`[server.js] err: ${err}`);
			util.error(`[server.js] Failed to start and/or connect to ${this.browser.name}. Please make sure ${this.browser.name} is running`);
			this.killBrowser();
			reject();
		});
	});
...
server.spawnBrowser = function() {
	...
	util.log(`[server.js] Starting '${this.browser.name}' with options: ${JSON.stringify(this.options)}`);
...
server.killBrowser = function() {
	util.log(`[server.js] Stopping ${this.browser.name}`);
...
server.restartBrowser = function() {
	this.isBrowserConnected = false;
	util.log(`[server.js] Restarting ${this.browser.name}`);
	this.browser.kill();
};
...
server.listenForBrowserClose = function() {
	...
	this.browser.onClose(() => {
		util.log('[server.js] Starting browser shutdown pipeline');
		this.isBrowserConnected = false;
		if(this.isBrowserClosing) {
			util.log(`[server.js] Stopped ${this.browser.name}`);
			return;
		}

		util.log(`[server.js] ${this.browser.name} connection closed... restarting ${this.browser.name}`);

		if (new Date().getTime() - start < 1000) {
			util.error(`[server.js] ${this.browser.name} died immediately after restart... stopping Prerender`);
			return process.exit();
		}
...
server.waitForBrowserToConnect = function() {
	return new Promise((resolve, reject) => {
		...
		let check = () => {
			if(++checks > 100) {
				return reject(`[server.js] Timed out waiting for ${this.browser.name} connection`);
			}
...
server.onRequest = function(req, res) {
	...
	util.log(`[server.js] getting '${req.prerender.url}' (req.url: ${req.url}) from ${req.headers['x-forwarded-for'] || req.connection.remoteAddress}`);
	if (!req.prerender.url) {
		util.error(`[server.js] no URL provided; headers=${JSON.stringify(req.headers)}`);
	...
	.then(() => {
		...
		if (!validUrl.isWebUri(encodeURI(req.prerender.url))) {
			util.error('[server.js] invalid URL:', req.prerender.url);
	...
	.then(() => {
		...
		setTimeout(() => {
			if (!req.prerender.responseSent) {
				util.error('[server.js] response not sent for', req.prerender.url);
			}
		...
		if (err) util.error(`[server.js] err: ${err}`);
...
server.finish = function(req, res) {
	...
	if (req.prerender.tab) {
		this.browser.closeTab(req.prerender.tab).catch((err) => {
			util.error('[server.js] error closing Chrome tab', err);
		});
	}
			...
			if (method) {
				try {
					cancellationToken = setTimeout(() => {
						util.error(`[server.js] Plugin event ${methodName} timed out (10s), layer index: ${index}, url: ${req?req.url:'-'}`);
					}, 10000);

					method.apply(layer, args);
				} catch (e) {
					util.error(`[server.js] caught error: ${e}`);
					next();
				}
			} else {
				next();
...
server._send = function(req, res) {
	...
	if (req.prerender.renderType == 'html') {
		Object.keys(req.prerender.headers || {}).forEach(function(header) {
			try {
				...
				util.error('[server.js] warning: unable to set header:', header);
			}
		});
	}
	...
	util.warn(`[server.js] got ${req.prerender.statusCode} in ${ms} for '${req.prerender.url}'`);
};
