...
server.startPrerender = function() {
	return new Promise((resolve, reject) => {
		...
		.catch((err) => {
			util.log(`[server.js] err: ${err}`);
			util.log(`[server.js] Failed to start and/or connect to ${this.browser.name}. Please make sure ${this.browser.name} is running`);
			this.killBrowser();
			reject();
		});
	});
...
server.listenForBrowserClose = function() {
	...
	this.browser.onClose(() => {
		...
		if (new Date().getTime() - start < 1000) {
			util.log(`[server.js] ${this.browser.name} died immediately after restart... stopping Prerender`);
			return process.exit();
		}
...
server.onRequest = function(req, res) {
	...
	if (!req.prerender.url) {
		util.log(`[server.js] no URL provided; headers=${JSON.stringify(req.headers)}`);
	...
	.then(() => {
		...
		if (!validUrl.isWebUri(encodeURI(req.prerender.url))) {
			util.log('[server.js] invalid URL:', req.prerender.url);
	...
	.then(() => {
		...
		setTimeout(() => {
			if (!req.prerender.responseSent) {
				util.log('[server.js] response not sent for', req.prerender.url);
			}
		...
		if (err) util.log(`[server.js] err: ${err}`);
...
server.finish = function(req, res) {
	...
	if (req.prerender.tab) {
		this.browser.closeTab(req.prerender.tab).catch((err) => {
			util.log('[server.js] error closing Chrome tab', err);
		});
	}
			...
			if (method) {
				try {
					cancellationToken = setTimeout(() => {
						util.log(`[server.js] Plugin event ${methodName} timed out (10s), layer index: ${index}, url: ${req?req.url:'-'}`);
					}, 10000);

					method.apply(layer, args);
				} catch (e) {
					util.log(`[server.js] ${e}`);
					next();
				}
			} else {
				next();
...
server._send = function(req, res) {
	...
	if (req.prerender.renderType == 'html') {
		Object.keys(req.prerender.headers || {}).forEach(function(header) {
			try {
				...
				util.log('[server.js] warning: unable to set header:', header);
			}
		});
	}
	...
	util.log(`[server.js] got ${req.prerender.statusCode} in ${ms} for '${req.prerender.url}'`);
};
