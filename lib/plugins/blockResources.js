      ...
      async ({ interceptionId, request, isNavigationRequest, ...rest }) => {
        ...
        blockedResources.forEach((substring) => {
          if (request.url.includes(substring)) {
            util.error(`[blockResources] !! BLOCKED: ${shortUrl} contains ${substring}`);
            shouldBlock = true;
          }
        });
        ...
        // );
        util.log(
          `[blockResources] isNavigationRequest: ${isNavigationRequest}, shouldBlock: ${shouldBlock}, interceptionId: ${interceptionId}, url: ${shortUrl}`,
        );
