...
module.exports = {
  get: (key, ...rest) =>  {
    if (!key) {
      util.error(`[levelCache] NOT getting; key is empty: '${key}' rest=${rest}`);
      return Promise.reject(new Error('key is empty'));
    }
  ...
  set: (key, value, options) => {
    if (!key) {
      util.error(`[levelCache] NOT setting; key is empty: '${key}' value='${value}' options=${options}`);
    ...
    util.log('[levelCache] setting', key, valueCopy, options);
